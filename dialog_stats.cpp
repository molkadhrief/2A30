...
void Dialog_stats::statistiques(QVector<double>* ticks,QVector<QString> *labels)
{
    QSqlQuery qry;
    int i=0;
    qry.exec("SELECT ID FROM GS_VOYAGEURS");
    while (qry.next())
    {
...
void Dialog_stats::MakeStat()
{
    ...
    // Couleurs
    A->setName("Repartition des VOYAGEURS selon le ID");
    ...
    ui->plot->yAxis->setLabel("NUM_VOY");
    ...
    QSqlQuery q1("SELECT NUM_VOY FROM GS_VOYAGEURS");
    while (q1.next())
    {
