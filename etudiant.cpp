...
bool Etudiant::ajouter()
{
    ...
    QString num=QString::number(NUM_VOY);
         ...
         query.prepare("INSERT INTO GS_VOYAGEURS(NOM,PRENOM,ID,NUM_VOY)" "VALUES (:NOM,:PRENOM,:ID,:NUM_VOY)");
...
bool Etudiant::supprimer(int ID)
{
    QSqlQuery query;
         ...
         query.prepare("DELETE FROM GS_VOYAGEURS where ID= :ID");
...
QSqlQueryModel *Etudiant::afficher()
{
    QSqlQueryModel *model=new QSqlQueryModel();
          ...
          model->setQuery("SELECT* FROM GS_VOYAGEURS");
...
bool Etudiant::modifier(int ID)
{
    ...
    query.prepare("UPDATE GS_VOYAGEURS set NOM=:NOM,PRENOM=:PRENOM,ID=:ID WHERE ID=:ID ");
...
model->setQuery("select * from GS_VOYAGEURS order by ID");
...
model->setQuery("select * from GS_VOYAGEURS order by PRENOM");
model->setHeaderData(0, Qt::Horizontal, QObject::tr("ID"));
model->setHeaderData(1, Qt::Horizontal, QObject::tr("PRENOM"));
model->setHeaderData(2, Qt::Horizontal, QObject::tr("NOM"));
model->setHeaderData(2, Qt::Horizontal, QObject::tr("NUM_VOY"));
...
model->setQuery("select * from GS_VOYAGEURS order by NOM");
model->setHeaderData(0, Qt::Horizontal, QObject::tr("ID"));
model->setHeaderData(1, Qt::Horizontal, QObject::tr("PRENOM"));
model->setHeaderData(2, Qt::Horizontal, QObject::tr("NOM"));
model->setHeaderData(2, Qt::Horizontal, QObject::tr("NUM_VOY"));
...
void Etudiant::chercheID(QTableView *table, int ID)
{
   ...
   query->prepare("select * from GS_VOYAGEURS where regexp_like(ID,:ID);");
   query->bindValue(":ID",ID);
   if(ID==0)
   {
